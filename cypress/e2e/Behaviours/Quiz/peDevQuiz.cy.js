...
describe('Quiz PE', () => {
    ...
    devices.forEach((device) => {
        context ('Execution of test cases for the QUIZ PE', () => {
            beforeEach(function(){
                cy.viewport(device)
            ...
            it('012 - Answer all the questions correctly and create the deal', () => {
                ...
                const poPeAppointment = new appointmentElements()
    
                cy.fixture('quizData').then(function(data){
                    this.data=data
                    poPeQuiz.getSeleccionaEdad().select(this.data.Edad2)
                    cy.get(this.data.Img1).click()
                    poPeQuiz.getCampoNombre().type(fakeFirstName)
                    cy.log(fakeFirstName)
                    poPeQuiz.getCampoApellido().type(fakeLastName)
                    cy.log(fakeLastName)
                    cy.addFakeMailPe()
                    cy.addCorrectNumberPe()
                    poPeQuiz.getCheckboxWaPe().check()
                    poPeQuiz.getFinalizarQuizButton().click()
